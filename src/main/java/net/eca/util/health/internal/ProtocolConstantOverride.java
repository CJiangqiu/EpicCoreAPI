...
import net.eca.util.health.internal.ProtocolDataflowAnalyzer.ConstProvenance;
 ...
 */
public final class ProtocolConstantOverride {

    private ProtocolConstantOverride() {}
    ...
    public static float resolveHealth(Object holder, float original) {
        ...
        if (DIAG_DUMPED.add(holder.getClass().getName())) {
            EcaLogger.info("[ProtocolConstantOverride] resolveHealth holder={} original={} override={}",
                    holder.getClass().getName(), original, override);
        }
        return override;
    }

    /* 写入 holder 的覆写血量。由数据流写入侧(dispatchWrite 的 ProtocolConstantOverrideSource 分支)调用。 */
    public static void setOverride(Object holder, float value) {
        if (holder != null) OVERRIDES.put(holder, value);
    }

    /* 读取 holder 当前覆写值，无则返回 null。供 ProtocolConstantOverrideSource.read() 经注入查表。 */
    ...
    private static final String SELF_INTERNAL = "net/eca/util/health/internal/ProtocolConstantOverride";
    ...
    public static byte[] transform(String classInternal, byte[] bytes) {
        ...
        try {
            ...
            if (targets.isEmpty()) {
                if (!LifeProtocolManager.isWarmupDiagnosticsSuppressed() && MISS_DUMPED.add(classInternal)) {
                    EcaLogger.info("[ProtocolConstantOverride] transform missed class={} sites={} missed={}",
            ...
            if (!LifeProtocolManager.isWarmupDiagnosticsSuppressed() && PATCH_DUMPED.add(classInternal)) {
                EcaLogger.info("[ProtocolConstantOverride] transform patched class={} sites={} patched={} fallback={} missed={}",
    ...
    /* 把树内 ProtocolConstantOverrideSource 的常数点登记并对其 owner 类触发 retransform 使 patch 生效。
       强制兼容模式或配置关闭时不登记 site，也不执行 retransform。没有 ProtocolConstantOverrideSource 时直接返回。 */
    public static void install(AnalysisResult tree) {
        ...
        for (Source s : tree.sources) {
            if (!(s instanceof ProtocolConstantOverrideSource co)) continue;
        ...
        if (!LifeProtocolManager.isWarmupDiagnosticsSuppressed() && INSTALL_DUMPED.add(installKey)) {
            EcaLogger.info("[ProtocolConstantOverride] install sites={} owners={}", siteCount, ownerInternals);
        }
        // 全部 site 登记完再逐 owner retransform：retransform 从原始字节码重跑全链，一次覆盖该类所有 site
        for (String internal : ownerInternals) {
            Class<?> owner = ProtocolDataflowAnalyzer.loadClass(internal);
            if (owner == null) {
                if (!LifeProtocolManager.isWarmupDiagnosticsSuppressed()
                        && INSTALL_DUMPED.add("missing:" + internal)) {
                    EcaLogger.info("[ProtocolConstantOverride] install skipped: owner missing {}", internal);
                }
                continue;
            }
            try {
                if (!EcaTransformerManager.retransformClass(owner)
                        && !LifeProtocolManager.isWarmupDiagnosticsSuppressed()
                        && INSTALL_DUMPED.add("unmodifiable:" + internal)) {
                    EcaLogger.info("[ProtocolConstantOverride] install skipped: owner retransform unavailable {}", owner.getName());
                }
            } catch (Throwable t) {
                if (t instanceof VirtualMachineError e) throw e;
                if (!LifeProtocolManager.isWarmupDiagnosticsSuppressed())
                    EcaLogger.info("[ProtocolConstantOverride] retransform failed owner={} msg={}", owner.getName(), t.toString());
            }
        }
    }
}
