 ...
 */
public final class ProtocolDataflowAnalyzer {
    ...
    private static final long DEFAULT_ANALYSIS_BUDGET_NANOS = 2_000_000_000L;
    ...
    private static final Map<Class<?>, AnalysisResult> SEMANTIC_SLICE_CACHE = new ConcurrentHashMap<>();
    private static final Set<Class<?>> SEMANTIC_SLICE_DIAG_DUMPED = ConcurrentHashMap.newKeySet();
    /* 单个扫描入口抛出的诊断去重(每类每入口一次) */
    private static final Set<String> EXTERNAL_ENTRY_FAILURE_DUMPED = ConcurrentHashMap.newKeySet();

    private ProtocolDataflowAnalyzer() {}

    public interface ClassBytesProvider {
        byte[] get(Class<?> clazz);
    }
    private static volatile ClassBytesProvider bytesProvider = ProtocolDataflowAnalyzer::defaultClassBytes;
    public static void setClassBytesProvider(ClassBytesProvider provider) {
        if (provider != null) bytesProvider = provider;
    }

    /* 覆写血量查表钩子：覆写表由外部持有，ProtocolConstantOverrideSource.read() 据此按 holder 取覆写值。
       未注入时恒无覆写，read 回退原常数。 */
    ...
    /* 语义交集入口：对 isAlive/isDeadOrDying 跑完整数据流管线，返回可写结构；
       无结果或无可写 Source 返回 null。写入由调用者落地。 */
    public static AnalysisResult resolveSemanticSliceResult(Class<?> entityClass) {
        if (entityClass == null) return null;
        AnalysisResult ar = SEMANTIC_SLICE_CACHE.computeIfAbsent(entityClass,
                c -> analyzeUnifiedSemanticSlice(entityClass));
        if (ar == null || ar.isEmpty() || ar.sources.isEmpty()) return null;
        return ar;
    }

    /* 仅查语义交集缓存、绝不触发分析(供运行期非阻塞查询)。重量级分析由调用者在后台线程经
       resolveSemanticSliceResult 预填，从而不阻塞服务器线程。 */
    public static AnalysisResult peekSemanticSliceResult(Class<?> entityClass) {
        if (entityClass == null) return null;
        AnalysisResult ar = SEMANTIC_SLICE_CACHE.get(entityClass);
        if (ar == null || ar.isEmpty() || ar.sources.isEmpty()) return null;
        return ar;
    }

    public static boolean verifySemanticDataflow(Expr root, LivingEntity entity, float expected, Source sink) {
        ...
        // ProtocolConstantOverride 在语义交集中是高危假阳性(覆写辅助方法常数不一定改变真实血量)，
        // 须额外通过 getHealth 校验；普通字段/SynchedData 写入信任 expressionMatches
        if (sink instanceof ProtocolConstantOverrideSource) {
            return LifeProtocolManager.verify(entity, expected);
        }
        return true;
    }
    ...
    private static boolean externalExpressionMatches(Expr expression, Source sink, float expected, EvalContext context) {
        ...
        if (!(value instanceof Number number)) {
            if (sink != null && EXTERNAL_EVAL_DIAG.add(sink.label + "|eval=" + value))
                EcaLogger.info("[SemanticSlice] eval non-numeric sink={} value={} expr={}", sink.label, value, expression);
            return false;
        }
        float actual = number.floatValue();
        boolean matches = ProtocolValueSemantics.matches(actual, expected);
        if (!matches && sink != null && EXTERNAL_EVAL_DIAG.add(sink.label + "|actual=" + actual))
            EcaLogger.info("[SemanticSlice] eval mismatch sink={} actual={} expected={}", sink.label, actual, expected);
        return matches;
    }
    ...
    /* 模糊观测源是互斥候选，不是必须同时写入的状态；每个分支因此持有自己的因果闭包。 */
    public record AuthorityBranch(Source authority, List<StoreWrite> maintenanceWrites,
                                  List<Source> dependencySources, List<Source> transactionSources) {
        public AuthorityBranch {
        ...
        public boolean hasMaintenanceWriter() {
            return !maintenanceWrites.isEmpty();
        }
    }

    /* 持久化传播属于协议，但不能冒充每 tick 都会执行的维护约束。 */
    public record ProtocolGraphResult(AnalysisResult observation, List<AuthorityBranch> authorityBranches,
                                      List<StoreWrite> persistenceWrites,
                                      List<Source> persistenceSources) {
        public ProtocolGraphResult {
            authorityBranches = List.copyOf(authorityBranches);
            persistenceWrites = List.copyOf(persistenceWrites);
            persistenceSources = List.copyOf(persistenceSources);
        }

        public List<Source> authoritySources() {
            return authorityBranches.stream().map(AuthorityBranch::authority).distinct().toList();
        }

        public List<StoreWrite> maintenanceWrites() {
            return authorityBranches.stream().flatMap(branch -> branch.maintenanceWrites().stream())
                    .distinct().toList();
        }

        public List<Source> dependencySources() {
            return Stream.concat(
                            authorityBranches.stream().flatMap(branch -> branch.dependencySources().stream()),
                            persistenceSources.stream())
                    .distinct().sorted(Comparator.comparing(source -> source.label)).toList();
        }

        public List<Source> transactionSources() {
            return authorityBranches.stream().flatMap(branch -> branch.transactionSources().stream())
                    .distinct().sorted(Comparator.comparing(source -> source.label)).toList();
        }

        public boolean hasMaintenanceWriter() {
            return authorityBranches.stream().anyMatch(AuthorityBranch::hasMaintenanceWriter);
        }
    }
    ...
    public static final class FieldChainSource extends Source {
        ...
        @Override public Object read(LivingEntity entity) {
            try {
                ...
                return cur;
            } catch (Throwable t) { if (t instanceof VirtualMachineError error) throw error; ProtocolDiagnostics.reflectionFailure("read", t); return null; }
        }
    ...
    public static final class ProtocolConstantOverrideSource extends Source {
        public final Expr receiver;
        public final float original;
        public final ConstProvenance provenance;

        public ProtocolConstantOverrideSource(Expr receiver, float original, ConstProvenance provenance) {
    ...
    // CompoundTag 的键值也是独立状态位置，容器与键都保留表达式身份以支持实例级事务。
    public static final class NbtValueSource extends Source {
        public final Expr containerExpr;
        public final Expr keyExpr;

        public NbtValueSource(Expr containerExpr, Expr keyExpr, Class<?> valueType) {
            super(valueType, "NBT:" + keyExpr);
            this.containerExpr = containerExpr;
            this.keyExpr = keyExpr;
        }
        ...
        public Object read(LivingEntity entity) {
            try {
                ...
                Object container = evaluate(containerExpr, context);
                Object key = evaluate(keyExpr, context);
                if (!(container instanceof CompoundTag tag) || !(key instanceof String name)) return null;
                if (valueType == boolean.class || valueType == Boolean.class) return tag.getBoolean(name);
                if (valueType == byte.class || valueType == Byte.class) return tag.getByte(name);
                if (valueType == short.class || valueType == Short.class) return tag.getShort(name);
                if (valueType == int.class || valueType == Integer.class) return tag.getInt(name);
                if (valueType == long.class || valueType == Long.class) return tag.getLong(name);
                if (valueType == float.class || valueType == Float.class) return tag.getFloat(name);
                if (valueType == double.class || valueType == Double.class) return tag.getDouble(name);
                if (valueType == String.class) return tag.getString(name);
                return tag.get(name);
            } catch (Throwable throwable) {
                if (throwable instanceof VirtualMachineError error) throw error;
                ProtocolDiagnostics.reflectionFailure("read", throwable);
                return null;
            }
        }

        @Override
        public void collectDescentAnchors(EvalContext context, Consumer<Object> sink) {
            sink.accept(read(context.entity()));
            sink.accept(safeEvaluate(containerExpr, context));
        }

        @Override
        protected String canonicalKey() {
            return "NBT:" + expressionKey(containerExpr) + ":" + expressionKey(keyExpr)
                    + ":" + valueType.getName();
        }
    }
    ...
    public static ProtocolSolveResult solveDetailed(Expr root, Source sink, Object target, EvalContext ctx) {
        if (root == null || sink == null) {
            return ProtocolSolveResult.failure(ProtocolSolveFailure.LOCATION_NOT_FOUND, "root or sink is null");
        }
        if (root instanceof StoreWrite write) {
            if (!sameSource(write.sink(), sink)) {
                return ProtocolSolveResult.failure(ProtocolSolveFailure.LOCATION_NOT_FOUND, "sink is absent from store write");
            }
            Object value = solveStoreWriteValue(write.valueExpr(), target, ctx);
            return value == null
                ? ProtocolSolveResult.failure(ProtocolSolveFailure.VALUE_NOT_REPRESENTABLE, "store value expression could not be evaluated")
                : ProtocolSolveResult.success(value);
        }
        if (sameSource(root, sink)) {
            Object value = coerceForType(target, sink.valueType);
            return value == null
                ? ProtocolSolveResult.failure(ProtocolSolveFailure.VALUE_NOT_REPRESENTABLE, sink.valueType.getName())
                : ProtocolSolveResult.success(value);
        }
        if (root instanceof Choice c) {
            ProtocolSolveResult last = ProtocolSolveResult.failure(ProtocolSolveFailure.LOCATION_NOT_FOUND, "sink is absent from all branches");
            for (Expr alt : c.alternatives()) {
                if (!containsSink(alt, sink)) continue;
                ProtocolSolveResult result = solveDetailed(alt, sink, target, ctx);
        ...
        if (root instanceof Op op) {
            ...
            if (idx == -2) {
                ...
                if (inv == null) return ProtocolSolveResult.failure(ProtocolSolveFailure.INVERTER_MISSING, "opcode=" + op.opcode());
                for (int i = 0; i < op.args().size(); i++) {
                    if (!containsSink(op.args().get(i), sink)) continue;
                    Object newT = inv.invert(target, op.args(), i, ctx);
                    if (newT == null) continue;
                    ProtocolSolveResult result = solveDetailed(op.args().get(i), sink, newT, ctx);
                    if (result.solved()) return result;
                }
                return ProtocolSolveResult.failure(ProtocolSolveFailure.MULTI_LOCATION_UNSUPPORTED, "sink occurs in multiple operands");
            }
            if (idx < 0) return ProtocolSolveResult.failure(ProtocolSolveFailure.LOCATION_NOT_FOUND, "sink is absent from operation");
            Inverter inv = TABLE.lookupOp(op.opcode());
            if (inv == null) return ProtocolSolveResult.failure(ProtocolSolveFailure.INVERTER_MISSING, "opcode=" + op.opcode());
            Object newT = inv.invert(target, op.args(), idx, ctx);
            if (newT == null) return ProtocolSolveResult.failure(ProtocolSolveFailure.VALUE_NOT_REPRESENTABLE, "operation inverse rejected target");
        ...
        if (root instanceof Call call) {
            ...
            if (idx == -2) {
                ...
                if (inv == null) {
                    ProtocolSolveFailure failure = call.owner().startsWith("java/util/function/")
                        ? ProtocolSolveFailure.CALL_NOT_RESOLVED : ProtocolSolveFailure.INVERTER_MISSING;
                    return ProtocolSolveResult.failure(failure, call.owner() + "#" + call.name() + call.desc());
                }
                for (int i = 0; i < call.args().size(); i++) {
                    if (!containsSink(call.args().get(i), sink)) continue;
                    Object newT = inv.invert(target, call.args(), i, ctx);
                    if (newT == null) continue;
                    ProtocolSolveResult result = solveDetailed(call.args().get(i), sink, newT, ctx);
                    if (result.solved()) return result;
                }
                return ProtocolSolveResult.failure(ProtocolSolveFailure.MULTI_LOCATION_UNSUPPORTED, "sink occurs in multiple call operands");
            }
            if (idx < 0) return ProtocolSolveResult.failure(ProtocolSolveFailure.LOCATION_NOT_FOUND, "sink is absent from call");
            Inverter inv = lookupCallInverter(call);
            if (inv == null) {
                ProtocolSolveFailure failure = call.owner().startsWith("java/util/function/")
                    ? ProtocolSolveFailure.CALL_NOT_RESOLVED : ProtocolSolveFailure.INVERTER_MISSING;
            ...
            if (newT == null) return ProtocolSolveResult.failure(ProtocolSolveFailure.VALUE_NOT_REPRESENTABLE, "call inverse rejected target");
            return solveDetailed(call.args().get(idx), sink, newT, ctx);
        }
        return ProtocolSolveResult.failure(ProtocolSolveFailure.CALL_NOT_RESOLVED, root.getClass().getSimpleName());
    }
    ...
    /* 方案 A 常数覆写重写：仅把返回值顶层、或 Choice 直接分支上的合格 float 常数
       替换为 ProtocolConstantOverrideSource，使常数点成为可写 sink；算术内部常数暂不处理。 */
    private static Expr rewriteProtocolConstantOverrides(Expr e) {
        ProtocolConstantOverrideSource top = asProtocolConstantOverride(e);
        if (top != null) return top;
        if (e instanceof Choice c) {
            boolean changed = false;
            List<Expr> rebuilt = new ArrayList<>(c.alternatives().size());
            for (Expr alt : c.alternatives()) {
                Expr rewritten = rewriteProtocolConstantOverrides(alt);
    ...
    private static ProtocolConstantOverrideSource asProtocolConstantOverride(Expr e) {
        ...
        return new ProtocolConstantOverrideSource(prov.receiver(), p.value().floatValue(), prov);
    }
    ...
    /* 用维护写入的下一状态替换单个权威源，计算该状态经过健康观测表达式后的值。 */
    public static Object evaluateWithSourceOverride(Expr expression, EvalContext context,
                                                    Source source, Object value) {
        if (expression instanceof Source current) {
            return current.equals(source) ? value : current.read(context.entity());
        }
        if (expression instanceof Primitive primitive) return primitive.value();
        if (expression instanceof Reference reference) return reference.value();
        if (expression == EntityParamMarker.I) return context.entity();
        if (expression instanceof OptionalContentExpr optional) {
            return unwrapOptionalContent(evaluateWithSourceOverride(
                    optional.optionalExpr(), context, source, value));
        }
        if (expression instanceof StoreWrite write) {
            return evaluateWithSourceOverride(write.valueExpr(), context, source, value);
        }
        if (expression instanceof Choice choice) {
            for (Expr alternative : choice.alternatives()) {
                Object result = evaluateWithSourceOverride(alternative, context, source, value);
                if (result != null) return result;
            }
            return null;
        }
        if (expression instanceof Op operation) {
            List<Object> arguments = new ArrayList<>(operation.args().size());
            for (Expr argument : operation.args()) {
                Object result = evaluateWithSourceOverride(argument, context, source, value);
                if (result == null) return null;
                arguments.add(result);
            }
            return execOp(operation.opcode(), arguments);
        }
        if (expression instanceof Call call) {
            ...
            for (Expr argument : call.args()) {
                Object result = evaluateWithSourceOverride(argument, context, source, value);
                if (result == null) return null;
                arguments.add(new Reference(result, result.getClass().getName().replace('.', '/')));
            }
            Call concrete = new Call(call.owner(), call.name(), call.desc(), arguments);
            Object known = evaluateKnownCall(concrete, context, null, false);
            return known != UnknownEval.INSTANCE ? known : invokeCall(concrete, context);
        }
        return null;
    }
    ...
    private static List<Expr> orderedWriteAlternatives(Choice choice) {
        ...
        alternatives.sort(Comparator.comparingInt(ProtocolDataflowAnalyzer::writeBranchScore).reversed());
        return alternatives;
    }
    ...
    private static void initDefaultRules() {
        ...
        // max/min 只有在目标没有越过另一操作数时才存在当前分支的逆。
        TABLE.registerCall("java/lang/Math", "max", "(FF)F",
            (t, args, idx, ctx) -> invertExtremum(t, args, idx, ctx, true, float.class));
        TABLE.registerCall("java/lang/Math", "min", "(FF)F",
            (t, args, idx, ctx) -> invertExtremum(t, args, idx, ctx, false, float.class));
        TABLE.registerCall("java/lang/Math", "max", "(II)I",
            (t, args, idx, ctx) -> invertExtremum(t, args, idx, ctx, true, int.class));
        TABLE.registerCall("java/lang/Math", "min", "(II)I",
            (t, args, idx, ctx) -> invertExtremum(t, args, idx, ctx, false, int.class));
        TABLE.registerCall("java/lang/Math", "max", "(DD)D",
            (t, args, idx, ctx) -> invertExtremum(t, args, idx, ctx, true, double.class));
        TABLE.registerCall("java/lang/Math", "min", "(DD)D",
            (t, args, idx, ctx) -> invertExtremum(t, args, idx, ctx, false, double.class));
    ...
    /* ==================== 语义交集：isAlive/isDeadOrDying 逆推真实血量 ==================== */

    /*
     * 通用数据流逆推骨架：从指定方法的指令帧提取血量表达式并生成 AnalysisResult。
     * getHealth 与语义交集各自只传入 extractor（从 MethodNode + 跑完的 Frame[] 中挑出 Expr），骨架完成其余公共部分。
     */
    private static AnalysisResult analyzeForHealthExpr(Class<?> owner, String name, String desc,
                                                        BiFunction<MethodNode, Frame<TaintValue>[], Expr> extractor) {
    ...
    private static AnalysisResult analyzeUnifiedSemanticSlice(Class<?> entityClass) {
        ...
        AnalysisResult combined = combineSemanticSliceCandidates(entityClass, candidates);
        if (SEMANTIC_SLICE_DIAG_DUMPED.add(entityClass)) {
            EcaLogger.info("[SemanticSlice] entity={} methods={} candidates={} sources={}",
                    entityClass.getName(), scannedMethods, candidates.size(),
                    combined != null ? combined.sources.size() : 0);
            EcaLogger.info("[SemanticSlice]   cost entity={} total={}ms perEntry={}",
    ...
    private static AnalysisResult safeExternalEntry(Class<?> entityClass, String label,
        ...
        try {
            ...
            if (EXTERNAL_ENTRY_FAILURE_DUMPED.add(entityClass.getName() + "|" + label)) {
                EcaLogger.info("[SemanticSlice] entry threw entity={} entry={} type={} msg={}",
    ...
    private static AnalysisResult combineSemanticSliceCandidates(Class<?> entityClass, List<AnalysisResult> candidates) {
        ...
        sources.sort(Comparator.comparingInt(ProtocolDataflowAnalyzer::externalSourcePriority)
    ...
    public record EffectiveProtocolRuntimeModel(Expr readExpr, Source storage, int predicate, float threshold) {}

    private static final Map<Class<?>, EffectiveProtocolRuntimeModel> EFFECTIVE_MODEL_CACHE = new ConcurrentHashMap<>();
    ...
    public static EffectiveProtocolRuntimeModel peekEffectiveProtocolRuntimeModel(Class<?> entityClass) {
    ...
    public static EffectiveProtocolRuntimeModel resolveEffectiveProtocolRuntimeModel(Class<?> entityClass, List<Source> candidates) {
        return resolveEffectiveProtocolRuntimeModel(entityClass, candidates, false);
    }

    /* 仅使用已缓存的比较表达式建模，不触发新的扫描，可在服务器线程直接调用。
       未命中时不记录失败签名：这只表示扫描尚未完成，而非该批候选分析不出模型。 */
    public static EffectiveProtocolRuntimeModel resolveCachedEffectiveProtocolRuntimeModel(Class<?> entityClass,
                                                                         List<Source> candidates) {
        return resolveEffectiveProtocolRuntimeModel(entityClass, candidates, true);
    }

    private static EffectiveProtocolRuntimeModel resolveEffectiveProtocolRuntimeModel(Class<?> entityClass, List<Source> candidates,
                                                                    boolean cachedOnly) {
        if (entityClass == null) return null;
        ...
        try {
            model = computeEffectiveProtocolRuntimeModel(entityClass, candidates, cachedOnly);
        } catch (Throwable t) {
            if (t instanceof VirtualMachineError e) throw e;
    ...
    /* 先扫语义交集所用的同一组方法，未得到模型再回退到全类扫描。
       优先段只分析含浮点比较指令的方法：血量判定必然表现为浮点比较，
       而 getter、注册逻辑等方法一条都没有，据此可跳过大部分方法及其依赖类字节码。 */
    private static EffectiveProtocolRuntimeModel computeEffectiveProtocolRuntimeModel(Class<?> entityClass, List<Source> evidence,
                                                                    boolean cachedOnly) {
        List<ComparisonFact> priority = cachedOnly
                ? PRIORITY_COMPARISON_CACHE.get(entityClass) : comparisonsOf(entityClass, true);
        if (priority != null) {
            EffectiveProtocolRuntimeModel model = selectEffectiveModel(entityClass, evidence, priority, "priority");
    ...
    private static EffectiveProtocolRuntimeModel selectEffectiveModel(Class<?> entityClass, List<Source> evidence,
                                                             List<ComparisonFact> comparisons, String stage) {
        Set<String> rejected = EFFECTIVE_MODEL_REJECTED.getOrDefault(entityClass, Set.of());
        EffectiveProtocolRuntimeModel best = null;
        ...
        for (ComparisonFact fact : comparisons) {
            ...
            for (Source candidate : evidence) {
                ...
                if (score > bestScore) {
                    bestScore = score;
                    best = new EffectiveProtocolRuntimeModel(expr, candidate, fact.predicate(), fact.threshold());
                }
            }
        }
        ...
        return new EffectiveProtocolRuntimeModel(pruneChoicesTo(best.readExpr(), best.storage()), best.storage(),
    ...
    public static void rejectEffectiveModel(Class<?> entityClass, EffectiveProtocolRuntimeModel model) {
    ...
    public static EffectiveProtocolRuntimeModel orientToAliveSide(EffectiveProtocolRuntimeModel model, LivingEntity entity) {
        ...
        return new EffectiveProtocolRuntimeModel(new Op(negationOpcode(model.readExpr()), List.of(model.readExpr())),
    ...
    private static List<ComparisonFact> collectClassComparisons(Class<?> entityClass, boolean priorityOnly) {
        ...
        for (MethodNode method : classNode.methods) {
            ...
            AnalysisCtx ctx = new AnalysisCtx(DEFAULT_MAX_DEPTH, deadline);
    ...
    private static Expr extractKnownWriteCall(MethodInsnNode call, List<Expr> args) {
        ...
        if (isNbtPut(call) && args.size() >= 3) {
            Class<?> valueType = descriptorToClass(Type.getArgumentTypes(call.desc)[1].getDescriptor());
            Source sink = new NbtValueSource(args.get(0), args.get(1),
    ...
    private static Expr tryInlineWriteCall(MethodInsnNode call, List<Expr> args, AnalysisCtx ctx, int depth) {
        ...
        /* 静态过程常被用作实体回调的实际实现。只要实体对象沿参数进入调用，就继续展开；
           这条判据来自对象身份传播，不依赖过程名称或所属模组。 */
        if (call.getOpcode() == Opcodes.INVOKESTATIC
                && !isHealthLifecycleCall(call) && !isEntityControlCall(call, args)) return null;
    ...
    private static Object invertExtremum(Object target, List<Expr> arguments, int sinkIndex,
                                         EvalContext context, boolean maximum, Class<?> valueType) {
        if (!(target instanceof Number targetNumber) || arguments.size() != 2
                || sinkIndex < 0 || sinkIndex > 1) return null;
        Object otherValue = context.eval(arguments.get(1 - sinkIndex));
        if (!(otherValue instanceof Number otherNumber)) return null;
        double expected = targetNumber.doubleValue();
        double boundary = otherNumber.doubleValue();
        if (maximum ? expected < boundary : expected > boundary) return null;
        return coerceForType(target, valueType);
    }

    private static boolean isNbtGet(MethodInsnNode call) {
        if (!call.owner.equals("net/minecraft/nbt/CompoundTag")) return false;
        return switch (call.name) {
            case "getBoolean", "m_128471_", "getByte", "m_128445_",
                    "getShort", "m_128448_", "getInt", "m_128451_",
                    "getLong", "m_128454_", "getFloat", "m_128457_",
                    "getDouble", "m_128459_", "getString", "m_128461_" -> true;
            default -> false;
        };
    }
    ...
    public static final class AnalysisResult {
        ...
        public Kind classify() {
            ...
            if (sources.stream().anyMatch(ProtocolConstantOverrideSource.class::isInstance)) return Kind.CONST_OVERRIDE;
        ...
        public static AnalysisResult of(Expr e, Class<?> definingClass) {
            Expr rewritten = rewriteProtocolConstantOverrides(e);
    ...
    /* 从健康观测反向取源，再从无参周期入口正向收集维护写入。
       CompoundTag 回调只描述持久化传播，不能据其存在误判为下一 tick 会覆盖状态。 */
    public static ProtocolGraphResult analyzeProtocolGraph(Class<?> entityClass) {
        AnalysisResult observation = analyze(entityClass);
        if (observation == null || observation.isEmpty() || observation.sources.isEmpty()) {
        ...
        for (StoreWrite write : recurringWrites) {
            ...
            Set<Source> outgoing = dependencies.computeIfAbsent(sink, ignored -> new HashSet<>());
            for (Source dependency : collectSources(write.valueExpr())) {
                outgoing.add(canonicalSource(dependencies.keySet(), dependency));
            }
        }

        List<AuthorityBranch> branches = new ArrayList<>();
        for (Source authority : observation.sources) {
            Set<Source> reachable = reachableSources(List.of(authority), dependencies);
            List<Source> dependencySources = new ArrayList<>(reachable);
            dependencySources.sort(Comparator.comparing(source -> source.label));
            ...
            for (Source candidate : reachable) {
                ...
                if (reachesAny(candidate, List.of(authority), dependencies)) {
                    transactionSources.add(candidate);
                }
            }
            transactionSources.sort(Comparator.comparing(source -> source.label));
            List<StoreWrite> maintenance = new ArrayList<>();
            for (StoreWrite write : recurringWrites) {
                if (transactionSources.contains(write.sink()) && !maintenance.contains(write)) {
        ...
        Map<Source, Set<Source>> persistenceDependencies = new HashMap<>();
        for (StoreWrite write : persistenceWrites) {
            Source sink = canonicalSource(persistenceDependencies.keySet(), write.sink());
            Set<Source> outgoing = persistenceDependencies.computeIfAbsent(sink, ignored -> new HashSet<>());
            for (Source dependency : collectSources(write.valueExpr())) {
                outgoing.add(canonicalSource(persistenceDependencies.keySet(), dependency));
            }
        }
        List<Source> persistenceSources = new ArrayList<>(
                reachableSources(observation.sources, persistenceDependencies));
        persistenceSources.sort(Comparator.comparing(source -> source.label));
        return new ProtocolGraphResult(observation, branches, persistenceWrites, persistenceSources);
    }
    ...
    private static Set<Source> reachableSources(Collection<Source> roots,
        ...
        Set<Source> reached = new HashSet<>();
    ...
    private static boolean reachesAny(Source start, Collection<Source> targets,
                                      Map<Source, Set<Source>> dependencies) {
        Set<Source> reached = reachableSources(List.of(start), dependencies);
    ...
    private static boolean isPersistenceOverride(Class<?> owner, MethodNode method) {
        if (!isVoidOverride(owner, method)) return false;
        for (Type argument : Type.getArgumentTypes(method.desc)) {
            if (Type.getDescriptor(CompoundTag.class).equals(argument.getDescriptor())) return true;
        }
        return false;
    }
    ...
    private static final BiFunction<MethodNode, Frame<TaintValue>[], Expr> COMPARISON_OPERANDS_EXTRACTOR = (mn, frames) -> {
    ...
    /* 解析结果缓存：深度内联会把同一批类反复解析上千次，而 EXPAND_FRAMES 展开栈映射帧是最贵的一步。
       ...
       只服务于只读分析路径；ProtocolConstantOverride 与 HeadBridge 注入会改指令，必须各自重新解析。 */
    ...
    private static final class AnalysisCtx {
        ...
        AnalysisCtx(int maxDepth, long deadlineNanos) {
            this.maxDepth = maxDepth;
            this.deadlineNanos = deadlineNanos;
        }
        void checkBudget() {
            if (nodeBudget <= 0 || System.nanoTime() > deadlineNanos) throw AnalysisBudgetExceeded.INSTANCE;
        }
        void consumeNode() {
            checkBudget();
            nodeBudget--;
        ...
        void consumeInline() {
            checkBudget();
            if (inlineBudget <= 0) throw AnalysisBudgetExceeded.INSTANCE;
            inlineBudget--;
        }
    }

    private static final class AnalysisBudgetExceeded extends RuntimeException {
        private static final AnalysisBudgetExceeded INSTANCE = new AnalysisBudgetExceeded();

        private AnalysisBudgetExceeded() {
    ...
    private static final class TaintInterpreter extends Interpreter<TaintValue> {
        ...
        @Override public TaintValue naryOperation(AbstractInsnNode insn, List<? extends TaintValue> values) {
            ...
            if (isNbtGet(m) && values.size() >= 2) {
                Class<?> valueType = descriptorToClass(retType.getDescriptor());
                return new TaintValue(sz, new NbtValueSource(
                        values.get(0).expr, values.get(1).expr,
                        valueType == null ? Object.class : valueType));
            }
    ...
    static Class<?> loadClass(String internalName) {
        ...
        for (ClassLoader cl : new ClassLoader[]{
                ...
                ProtocolDataflowAnalyzer.class.getClassLoader()
        }) {
    ...
    public static ProtocolSolveResult buildWritePath(Expr root, Source sink, Object target, EvalContext ctx) {
    ...
    private static void collectWriteCandidates(Expr root, Source sink, Object target, EvalContext ctx,
        ...
        ProtocolSolveResult result = solveDetailed(root, sink, target, ctx);
