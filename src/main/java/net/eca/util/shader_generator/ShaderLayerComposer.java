...
public final class ShaderLayerComposer {
    ...
    public static String compose(List<ShaderLayer> layers) {
        ...
        source
            ...
            .append("}\n\n")
            .append("float ecaFbm(vec2 value, int octaves) {\n")
            .append("    float result = 0.0;\n")
            .append("    float amplitude = 0.5;\n")
            .append("    for (int i = 0; i < 6; i++) {\n")
            .append("        if (i >= octaves) break;\n")
            .append("        result += amplitude * ecaNoise(value);\n")
            .append("        value *= 2.0;\n")
            .append("        amplitude *= 0.5;\n")
            .append("    }\n")
            .append("    return result;\n")
            .append("}\n\n")
            .append("float ecaArcDistance(vec2 p, float r, float a1, float a2) {\n")
            .append("    float a = atan(p.y, p.x);\n")
            .append("    if (a < a1) a += 6.28318530718;\n")
            .append("    if (a > a2) a -= 6.28318530718;\n")
            .append("    if (a >= a1 && a <= a2) {\n")
            .append("        return abs(length(p) - r);\n")
            .append("    }\n")
            .append("    vec2 p1 = r * vec2(cos(a1), sin(a1));\n")
            .append("    vec2 p2 = r * vec2(cos(a2), sin(a2));\n")
            .append("    return min(length(p - p1), length(p - p2));\n")
            .append("}\n\n")
            ...
            .append("}\n\n")
            .append("float ecaStarDistance(vec2 point, int points, float outerR, float innerR) {\n")
            .append("    float sector = 6.2831853 / float(points);\n")
            .append("    float halfSector = sector * 0.5;\n")
            .append("    float best = 1e10;\n")
            .append("    for (int i = 0; i < 24; i++) {\n")
            .append("        if (i >= points * 2) break;\n")
            .append("        float fi = float(i);\n")
            .append("        float ai = fi * halfSector;\n")
            .append("        float ri = (mod(fi, 2.0) < 0.5) ? outerR : innerR;\n")
            .append("        int j = i + 1;\n")
            .append("        float fj = float(j);\n")
            .append("        float aj;\n")
            .append("        float rj;\n")
            .append("        if (j >= points * 2) {\n")
            .append("            aj = 0.0;\n")
            .append("            rj = outerR;\n")
            .append("        } else {\n")
            .append("            aj = fj * halfSector;\n")
            .append("            rj = (mod(fj, 2.0) < 0.5) ? outerR : innerR;\n")
            .append("        }\n")
            .append("        vec2 vi = vec2(cos(ai), sin(ai)) * ri;\n")
            .append("        vec2 vj = vec2(cos(aj), sin(aj)) * rj;\n")
            .append("        float d = ecaSegmentDistance(point, vi, vj);\n")
            .append("        best = min(best, d);\n")
            .append("    }\n")
            .append("    return best;\n")
            .append("}\n\n")
