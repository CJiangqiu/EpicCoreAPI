...
- `/eca setFilter <目标> true <类型>` - 为玩家施加屏幕滤镜（类型：sketch、spotlight、matrix、rain、desert、snow、toxic）
...
在 Forge 事件总线和 MC tick 循环之外启动一个独立守护线程，持续监控被追踪的实体，一旦死亡或容器注册丢失就自动复活——比一次性的 `reviveAllContainers` 调用提供更强的存活保障。

**命令**（需要 OP 权限等级 2）：
- `/eca resurrection start` — 启动守护线程
- `/eca resurrection stop` — 停止守护线程
- `/eca resurrection status` — 查看线程状态及累计复活/检查次数
- `/eca resurrection add <目标>` — 将实体加入追踪
- `/eca resurrection remove <目标>` — 将实体从追踪中移除
- `/eca resurrection list` — 列出所有被追踪实体及各容器状态
- `/eca resurrection check <目标>` — 对实体进行一次性容器完整性检查（不执行复活）
- `/eca resurrection revive <目标>` — 立即手动强制复活被追踪的实体
- `/eca resurrection interval <毫秒>` — 设置轮询间隔（100~10000 ms，默认 25 ms）

**直接 API**（未通过 `EcaAPI` 暴露，需 `import net.eca.util.ResurrectionManager`）：
...
> 守护线程以略低于正常的优先级运行，默认轮询间隔 25 ms，即被追踪实体在死亡后约 25 ms 内即被复活。请勿用于会大量生成的实体。

### 实体扩展

为指定实体类型附加特殊效果：创建继承 `EntityExtension` 的子类，并在类上标注 `@RegisterEntityExtension` 即可注册。
...
为指定物品附加着色器渲染效果：创建继承 `ItemExtension` 的子类，并在类上标注 `@RegisterItemExtension` 即可注册。物品扩展完全是客户端行为，无需任何网络同步。
...
ECA 内置了若干供实体扩展和物品扩展使用的着色器预设，可直接在扩展中使用对应的 RenderType——把上面示例里的 `CustomRenderTypes` 换成预设名字即可。每个预设提供 4 个 RenderType：实体扩展用的 `BOSS_BAR`、`BOSS_LAYER`、`SKYBOX`，以及物品扩展用的 `ITEM`。
...
一组全屏后处理滤镜，可由服务端按玩家施加，通过命令或 API 均可。滤镜会同步到客户端，作为一道着色器 pass 叠加在世界画面上。每个玩家同一时刻只能激活一个滤镜——施加新滤镜会替换当前滤镜——玩家退出登录时其滤镜会自动清除。
