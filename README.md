...
The Resurrection System runs a dedicated daemon thread outside the Forge event bus and MC tick loop. It continuously monitors a set of tracked entities and auto-revives any that die or lose container registration, providing a stronger liveness guarantee than a one-shot `reviveAllContainers` call.
...
线程复活系统会在 Forge 事件总线和 MC tick 循环之外启动一个独立守护线程，持续监控被追踪的实体，并在其死亡或容器注册丢失时自动复活——提供比一次性 `reviveAllContainers` 更强的存活保障。
